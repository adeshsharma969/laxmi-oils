  ...
  return (
    <div data-testid="account-page" className="px-4 sm:px-5 md:px-10 py-6 md:py-10">
      <div className="border-b-[3px] border-[#1F3D2B] pb-4 md:pb-6 mb-6 md:mb-8 flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] text-[#B8431A]">Namaste</div>
          ...
          <div className="border-[3px] border-[#1F3D2B] bg-[#F5F1E8] p-4 sm:p-5 brutal-shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] sm:text-xs text-[#1F3D2B]/70 truncate">{user.email}</div>
              </div>
            </div>
              ...
              {orders.map(o => (
                <div key={o.order_id} data-testid={`order-${o.order_id}`} className="border-[3px] border-[#1F3D2B] bg-[#F5F1E8] p-4 sm:p-5 brutal-shadow-sm">
                  <div className="flex flex-wrap justify-between gap-3 border-b-2 border-[#1F3D2B]/20 pb-3 mb-3">
                    <div>
                      <div className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-[#1F3D2B]/60">Order</div>
                      <div className="font-mono font-black text-base sm:text-lg text-[#1F3D2B]">{o.order_id}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-[#1F3D2B]/60">{new Date(o.created_at).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})}</div>
                      <div className={`inline-block mt-1 px-2 py-0.5 text-[8px] sm:text-[10px] font-black uppercase tracking-wider border-2 ${o.status==='delivered'?'bg-[#1F3D2B] text-[#D98F00] border-[#1F3D2B]':'bg-[#D98F00] text-[#1F3D2B] border-[#1F3D2B]'}`}>{o.status}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between mt-3 pt-3 border-t-2 border-[#1F3D2B]/20">
                    <span className="text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] text-[#1F3D2B]/60">Total</span>
                    <span className="font-display font-black text-lg sm:text-xl text-[#1F3D2B]">₹{o.total}</span>
                  </div>
                  {o.tracking?.trackingUrl && (
                    <div className="mt-2 text-[10px] sm:text-xs font-black uppercase tracking-widest text-[#1F3D2B]">
                  ...
                  <div className="mt-3 flex justify-end">
                    <Link to={`/invoice/${o.order_id}`} data-testid={`invoice-${o.order_id}`} className="touch-target-sm text-[10px] sm:text-xs font-black uppercase tracking-widest border-2 border-[#1F3D2B] px-3 py-1.5 bg-[#F5F1E8] hover:bg-[#D98F00]">View Invoice →</Link>
                  </div>
                </div>
              ))}
            </div>
...
function Stat({ label, value, mono }) {
  return (
    <div className="bg-[#F5F1E8] border-2 border-[#1F3D2B] px-2 sm:px-3 py-2">
      ...
      <div className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.25em] text-[#1F3D2B]/70">{label}</div>
    </div>
  );
}
