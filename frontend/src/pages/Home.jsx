        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {CATEGORIES.map((c,i)=>(
            <motion.div key={c.slug} initial={{opacity:0,y:40}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5, delay:i*0.1, ease:[0.22,1,0.36,1]}}>
              <Link data-testid={`category-${c.slug}`} to={`/products?cat=${c.slug}`} className="block border-[3px] border-[#1F3D2B] brutal-shadow hover:-translate-y-1 hover:shadow-[6px_6px_0_0_#1F3D2B] hover:scale-[1.02] transition-all relative overflow-hidden h-[160px] xs:h-[180px] sm:h-[220px] md:h-[260px]" style={{background:c.bg}}>
                <div className="absolute inset-0 w-full h-full p-4">
                  <Image src={c.image} alt={c.name} fill className="object-contain" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 50vw, 25vw" />
                </div>
