    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.div data-testid="search-palette" initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="fixed left-1/2 top-[8vh] -translate-x-1/2 w-[92vw] max-w-xl bg-[#F5F1E8] border-[3px] border-[#1F3D2B] shadow-[8px_8px_0_0_#1F3D2B] z-50 overflow-hidden">
            <div className="flex items-center gap-3 border-b-[3px] border-[#1F3D2B] px-4 py-3 bg-[#D98F00]">
              ...
              <input data-testid="search-input" autoFocus value={q} onChange={e=>setQ(e.target.value)} onKeyDown={onKey} placeholder="Search oils, categories, recipes…" className="flex-1 bg-transparent outline-none font-bold text-[#1F3D2B] placeholder:text-[#1F3D2B]/60"/>
              <button onClick={onClose} className="touch-target-sm w-9 h-9 border-2 border-[#1F3D2B] flex items-center justify-center"><X size={12} strokeWidth={3}/></button>
            </div>
              ...
              {results.map((p, idx) => (
                <button key={p.product_id} data-testid={`search-res-${p.product_id}`} onClick={()=>go(p)} onMouseEnter={()=>setI(idx)} className={`w-full flex items-center gap-3 px-4 py-3 border-b-2 border-[#1F3D2B]/10 text-left ${i===idx?"bg-[#D98F00]/30":""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs font-black uppercase tracking-[0.18em] text-[#1F3D2B]/80">{p.category} · from ₹{p.sizes[0].price}</div>
                  </div>
